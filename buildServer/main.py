...
cmd=[os.getenv('CMD')]
print(f"Orrignal cmd: {cmd}")
cmd_list = cmd[0].split('#')
cmd=f'["{cmd_list[0]}", "{cmd_list[1]}"]'
print(f"Command: {cmd}")
...
def detect_project_type(files):
    if any(file in files for file in ['index.html', 'webpack.config.js', 'vite.config.js', 'frontend']):
        return 'frontend'
