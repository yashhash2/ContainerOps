...
def run(request):
    if request.method == "POST":
        try:
            ...
            docker_user = body.get("dockerUser", None)
            docker_pass = body.get("dockerPass", None)
            image_name = body.get("imageName", None)
            ...
            if not all([github_url, docker_user, docker_pass, image_name]):
                return JsonResponse({
                    "status": "error",
                    "message": "Missing required parameters (githubUrl, dockerUser, dockerPass, imageName)."
                })
